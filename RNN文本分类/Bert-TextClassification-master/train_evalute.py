...
def train(epoch_num, model, train_dataloader, dev_dataloader, 
...
output_model_file, output_config_file, log_dir, print_step, early_stop):
    ...
    Args: 
        ...
        early_stop: 提前终止
    """
    print('print_step',print_step)
    print('len(train_dataloader)',len(train_dataloader),'len(dev_dataloader)',len(dev_dataloader))
            ...
            if (step + 1) % gradient_accumulation_steps == 0:
                ...
                if global_step % print_step == 0 and global_step != 0:
                    ...
                    #     best_auc = dev_auc
                        ...
                        early_stop_times += 1
        #每个epoch 输出一次指标
        print('train_loss:%.3f, dev_loss:%.3f, dev performance: acc:%.3f, f1:%.3f' % (train_loss,dev_loss,np.mean(accs),np.mean(f1s)))
