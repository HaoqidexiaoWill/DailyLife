...
tokenizer = BertTokenizer.from_pretrained('/home1/lsy2018/NL2SQL/python5/chinese_wwm_L-12_H-768_A-12/vocab.txt')
bert = BertModel.from_pretrained('/home1/lsy2018/NL2SQL/python5/chinese_wwm_L-12_H-768_A-12/')


class RegexEgine:
    @staticmethod
    def conds_rewrite(question,table_id,header_list,tableID2question):
        print(question)
        print(header_list)
        # print(tableID2question)
        if table_id in tableID2question:
            print(tableID2question[table_id])
            question_match = RegexEgine.query_match(question,[query_dict['sample_question'] for query_dict in tableID2question[table_id]])
            for each in tableID2question[table_id]:
                if question_match == each['sample_question']:
                    return each['tmp_sql_dict']['conds']
        else:
            # TODO
            print('no tableid  in history')
            pass
        return 

    @staticmethod
    def query_match(user_question,question_candidate_list):
        result_dict = {question:RegexEgine.sim(user_question,question) for question in question_candidate_list}
        print(result_dict)
        best_match_question = sorted(result_dict.items(), key=lambda x: x[1], reverse=True)[0][0]
        print(best_match_question)

        return best_match_question

    @staticmethod
    def sim(string1,string2,bert = bert,tokenizer = tokenizer):
        bert.eval()
...
class Predictor:
    ...
    def evaluate(self, logits_lists, cls_index_list, labels_lists, question_list, table_id_list, sample_index_list, correct_sql_list, table_dict):
        ...
        for i in range(len(sample_index_list)):
            ...
            connection_list = []
            value_change_list = []
            if 1 not in sample_type_logits:
                # TODO 目前使用trick 来代替的
                pass

            for j, col_type in enumerate(sample_type_logits):
                '''
                如果col_type是0表示当前列名在sel语句中，如果是1的话在where的语句中，如果是2就不管他不使用该列
                j 是表中的每一列对应的下标索引
                '''
                if col_type == 0:
                    # sel
                    ...
                    '''
                    sample_question : 长沙2011年平均每天成交量是3.17，那么近一周的成交量是多少
                    tag_list: 当前问题的序列标注，用来标记作为条件的值和运算符，其中tag = 4 表示不被抽取
                    '''


                    # 当前预测的列对应的 cond的数量
                    con_num = sample_con_num_logits[j]
                    # 当前预测的列对应的 cond 之间的操作符op
                    ...
                    for i in range(0, len(tag_list)):
                        ...
                        # 4代表不选取当前的问题中的当前字
                        if current_tag == 4:
                            # 如果当前的字标注是4->不选择，而他的前一个字符不是4，表示当前value的预测是截止到上一个字符停止
                            if previous_tag in [0, 1, 2, 3]:
                                candidate_list.append([[], []])
                                candidate_list_index += 1
                        else:
                            # 当前的字标注不是4，将字符添加到候选列表当中
                    ...
                    for i in range(len(value_start_index_list)):
                        # i 是预测的value的起始索引，表示遍历考虑每个每次的候选value
                        candidate = candidate_list[i]
                        '''
                        i:0, candidate: [['3', '.', '1', '7'], [2, 2, 2, 2]] 
                        '''
                        ...
                        # print('col_data_type',col_data_type)    # col_data_type real
                        # print(col_values)                       # ['6.95', '2.89', '3.29', '3.17', '0.87', '1.62', '0.97', '1.33', '3.96', '12.23', '28.51']
                        ...
                        if (con_col == 2 and op == 2 and value_str == "1000") or \
                            ...
                            print(1)
                        """
                        # 下面几行代码是通过根据上面找到的value集合来通过找最长的数字以及汉字来创建候选集合
            ...
            if len(tmp_sql_dict["conds"]) == 0:
                # print(sample_question)
                # print('tmp_sql_dict',tmp_sql_dict)
                # print('sample_sql',sample_sql)
                # # 检查一下字的标注序列
                # print('sample_tag_logits',sample_tag_logits)
                # print('sample_table_id',sample_table_id)
                print(sample_question,tmp_sql_dict["conds"],sample_type_logits)
                conds_rewrite = RegexEgine.conds_rewrite(sample_question,sample_table_id,table_header_list,tableID2question)
                print('conds_rewrite',conds_rewrite)
                if conds_rewrite:
                    tmp_sql_dict['conds'] = conds_rewrite
                # exit()
            else:
                if  sample_table_id not in tableID2question:
                    tableID2question [sample_table_id] = [{'sample_question':sample_question,'tmp_sql_dict':tmp_sql_dict}]
                else:
                    tableID2question [sample_table_id].append({'sample_question':sample_question,'tmp_sql_dict':tmp_sql_dict})
    ...
    def inference(self, do_evaluate=True, do_test=False):
        ...
        model.load_state_dict(torch.load("/home1/lsy2018/NL2SQL/python5/model_save/my_model.bin"))
...
if __name__ == "__main__":
    ...
    time1 = time.time()
    predictor.inference(do_evaluate=True, do_test=False)
