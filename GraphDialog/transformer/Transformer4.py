...
class TableSemanticDecoder(nn.Module):
    ...
    def __init__(self, vocab_size, d_word_vec, n_layers, d_model, n_head, dropout=0.1):
        ...
        self.prob_proj = nn.Linear(d_model,1)
    ...
    def forward(self, tgt_seq, src_seq, act_vecs):
        ...
        dec_inp, _, _ = self.post_layer_stack(argument_vecs, dec_inp, enc_output, 
                                                ...
                                                slf_attn_mask=slf_attn_mask,
                                                dec_enc_attn_mask=dec_enc_attn_mask)            
        dec_inp, _, dec_enc_attn = self.final_layer_stack(dec_inp, enc_output, 
                                                non_pad_mask=non_pad_mask,
                                                slf_attn_mask=slf_attn_mask,
                                                dec_enc_attn_mask=dec_enc_attn_mask)
        logits = self.tgt_word_prj(dec_inp)
        # torch.Size([348, 39, 3130]) 
        # print(logits.size())
        # print(logits)
        # print(src_seq)
        # print(src_seq.size())               #torch.Size([512, 100]),100 maxlen
        # print(tgt_seq.size())               #torch.size([512, 39]),39 maxlen
        # # print(tgt_seq)
        # print(dec_enc_attn.size())          # torch.Size([512, 39, 100])
        
        src_seq = src_seq.unsqueeze(1).expand_as(dec_enc_attn)
        # print('src_seq',src_seq.size())
        # print('dec_enc_attn',dec_enc_attn.size())
        # print(dec_enc_attn)

        final_dist = torch.zeros_like(logits)
        final_dist.scatter_(dim =2,index = src_seq,src = dec_enc_attn)
        prob = torch.sigmoid(self.prob_proj(dec_inp)) 
        final_dist = (1-prob)*final_dist+prob*logits
        # print('succ')
        # exit()
