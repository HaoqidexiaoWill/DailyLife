...
def run_cnn_4(self,inputs):
    ...
    outputs = outputs.squeeze(3).transpose(1, 2)  #50 30  000

    outputs.contiguous().view(-1, outputs.size(2))  #1500 400
    outputs = self.mapping(outputs)   # 1500 800
...
class PositionalEmbedding(nn.Embedding):
    ...
    def get_absolute_pos(self,word_sequences):
        ...
        for word_seq in word_sequences:
            start_idx = 1
            word_pos = []
            #print(word_seq.size())              #torch.Size([5])
            for pos in word_seq:
                if int(pos) == 0:
                    word_pos.append(0)
                else:
    ...
    def forward(self,*args, **kwargs):          #args 是一个张量元组
        ...
        word_pos_encoded = word_embeddings + F.sigmoid(self.position_enc(word_pos.cuda()))      
        #word_pos_encoded = word_embeddings              
        return word_pos_encoded             #torch.Size([7, 5, 400])
...
class GCEencoder(nn.Module):
    ...
    def __init__(self, din, dhid, slots, dropout=None):
        ...
        self.conv_final2 = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=400, kernel_size=(1,800),stride=1, padding=self.padding),nn.ReLU(),)
    ...
    def forward(self, x, x_len, slot, slot_emb, default_dropout=0.2):
        ...
        #h_   = self.global_selfattn(h, x_len, history)
        return h, c
...
class Model(nn.Module):
    ...
    def __init__(self, args, ontology, vocab):
        ...
        self.emb_fixed = PositionalEmbedding(num_embeddings = len(vocab),embedding_dim = args.demb, dropout=args.dropout.get('emb', 0.2))
