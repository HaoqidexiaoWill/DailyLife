...
def run_cnn(self,inputs,inputs_,Layers = 1,attention_type = 'att_add',feature_size ='13'):
    inputs = inputs.permute(0, 2, 1)
    out = [conv(inputs) for conv in self.convs]
    out = torch.cat(out, dim=1)
    if Layers == 1:
        intputs_resNet1 = out.transpose(1,2)+inputs_
        outputs_resNet1_A = self.conv1(intputs_resNet1.permute(0, 2, 1)).transpose(1,2)
        outputs_resNet1_B = self.conv3(intputs_resNet1.permute(0, 2, 1)).transpose(1,2)
        outputs_resNet1_C = self.conv5(intputs_resNet1.permute(0, 2, 1)).transpose(1,2)
        ...
        if attention_type == 'att_add':
            if feature_size == '13':
                attn = attn13
            elif feature_size == '15':
                attn = attn15
            elif feature_size == '35':
                attn =attn35
            elif feature_size == 'add_all':
                attn = attn13+attn15+attn35
        elif attention_type == 'att_concat':
            if feature_size == '1315':
                attn_ = torch.cat(([attn13,attn15]),dim =1)
                attn  = self.attention_(attn_)
            elif feature_size =='1335':
                attn_ = torch.cat(([attn13,attn35]),dim =1)
                attn  = self.attention_(attn_)
            elif feature_size =='1535':
                attn_ = torch.cat(([attn15,attn35]),dim =1)
                attn  = self.attention_(attn_)
            elif feature_size == 'concat_all':
                attn_ = torch.cat(([attn13,attn15,attn35]),dim =1)
                attn  =self.attention(attn_)                       
                
        outputs_resNet1 = attn.view(inputs_.size(0), inputs_.size(1), -1)
        outputs_resNet1 = outputs_resNet1 + intputs_resNet1
...
def run_cnn(self,inputs,inputs_,x_new = False,Gate = True,SameKernel = False,Layers = 2,isResNet = True):
    ...
    #异核1层
    if isResNet == True and Gate == True and Layers == 1 and SameKernel == False:
        ...
        # print(attn13.size())          torch.size(1500,400)
        # print(attn15.size())          torch.size(1500,400)
        # print(attn35.size())          torch.size(1500,400)
...
class GCEencoder(nn.Module):
    ...
    def __init__(self, din, dhid, slots, dropout=None):
        ...
        self.attention_ = nn.Linear(800,400)
    ...
    def forward(self, x, x_len, slot, slot_emb, default_dropout=0.2):
        ...
        global_h = run_cnn(self,inputs=x_new,inputs_=x,Layers=1,attention_type = 'att_add',feature_size = '13')        #异核1层
        # global_h = run_cnn(self,inputs=x_new,inputs_=x,Layers=1,attention_type = 'att_add',feature_size = '15')        #异核1层
        # global_h = run_cnn(self,inputs=x_new,inputs_=x,Layers=1,attention_type = 'att_add',feature_size = 'add_all')        #异核1层
        # global_h = run_cnn(self,inputs=x_new,inputs_=x,Layers=1,attention_type = 'att_concat',feature_size = '1315')        #异核1层
        # global_h = run_cnn(self,inputs=x_new,inputs_=x,Layers=1,attention_type = 'att_concat',feature_size = '1535')        #异核1层
        # global_h = run_cnn(self,inputs=x_new,inputs_=x,Layers=1,attention_type = 'att_concat',feature_size = '1335')        #异核1层      
        # global_h = run_cnn(self,inputs=x_new,inputs_=x,Layers=1,attention_type = 'att_concat',feature_size = 'concat_all')        #异核1层
