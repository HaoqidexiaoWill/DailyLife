...
def run_rnn(rnn, inputs, lens):
    ...
    return recovered                                                        ##torch.Size([50, 30, 400])


def run_cnn_1(cnn,input):
    #print(input.size()) #torch.Size([50, 30, 400])
    outputs = input.unsqueeze(1) # batch, 1, seq_len, 2*hidden              ##torch.Size([50, 1, 30, 400])
    outputs = cnn(outputs) # batch, out_channels, seq_len, 1                ##torch.Size([50, 400, 30, 1])
    outputs = F.relu(outputs)                                               #要记得把kernel_size的第二个维度设为embedding_size一样
                                                                            #这样就会变成上下滑动的卷积
    outputs = outputs.squeeze(3).transpose(1,2) # batch, seq_len, 2*hidden  ##torch.Size([50, 30, 400, 1])
    return outputs

def run_cnn(self,cnn,input):
    #print(input.size()) #torch.Size([50, 30, 400])
    #input = affine(input)                                                   ##torch.Size([50, 30, 800])
    for i in range (1):
        ...
        #outputs = self.maxpooling(outputs)                                            #要记得把kernel_size的第二个维度设为embedding_size一样
                                                                                #这样就会变成上下滑动的卷积
        outputs = outputs.squeeze(3).transpose(1,2) # batch, seq_len, 2*hidden  ##torch.Size([50, 30, 400, 1])
        A, B = outputs.split(200, 2)                                            # A, B: batch, seq_len, hidden
...
class GLADEncoder(nn.Module): 
    ...
    def __init__(self, din, dhid, slots, dropout=None):
        ...
        #self.kernel = (self.kernel_size, self.hidden_size * 2)
        self.stride = 1
        # self.padding = (int((self.kernel_size -1) / 2), 0)
        self.padding = (1//2, 0)
        ...
        for s in slots:
            ...
            setattr(self, '{}_cnn'.format(s), self.conv)
    ...
    def forward(self, x, x_len, slot, default_dropout=0.2):
        ...
        local_cnn = getattr(self, '{}_cnn'.format(slot))
        ...
        local_h = run_cnn(self,local_cnn, x)





        # NOTE:dropout->torch.nn.functional.dropout(input, p=0.5, training=False, inplace=False)
