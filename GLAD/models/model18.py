...
class Model(nn.Module):
    ...
    def __init__(self, args, ontology, vocab):
        ...
        self.utt_scorer2 = nn.Linear(2 * args.dhid, 1)
    ...
    def forward(self, batch):
        ...
        system_transcript, system_transcript_len = pad([e.num['system_transcript'] for e in batch], self.emb_fixed, self.device, pad=eos)
        ...
        for s in self.ontology.slots:
            ...
            H_sys, c_sys = self.utt_encoder(system_transcript, system_transcript_len, slot=s, slot_emb=s_emb)
            ...
            # print(H_sys.size())         #torch.Size([batch_size, 35, 400])
            # print(c_sys.size())         #torch.Size([batch_size, 1, 400])
            ...
            for c_val in C_vals:
                c_val = c_val.squeeze(0)
                q_sys, _ = attend(H_sys, c_val.unsqueeze(0).expand(len(batch), *c_val.size()), lens=system_transcript_len)
                q_syss.append(q_sys)
            y_syss = self.utt_scorer2(torch.stack(q_syss, dim=1)).squeeze(2)

            ys[s] = F.sigmoid(y_utts + self.score_weight * y_syss)
